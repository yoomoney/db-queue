...
public class QueueStatisticsDao {
    ...
    public Map<String, Long> getPendingTasksCount(String tableName) {
        ...
        jdbcTemplate.query("SELECT queue_name, COUNT(1) FROM tasks WHERE (attempt > 1) OR " +
                "(attempt = 1 AND process_time < now()) " +
                "GROUP BY queue_name ORDER BY queue_name", rs -> {
    ...
    public Map<String, Long> getTasksCount(String tableName) {
        ...
        jdbcTemplate.query("SELECT queue_name, COUNT(1) FROM tasks GROUP BY queue_name ORDER BY queue_name", rs -> {
    ...
    public Map<String, List<TaskRecord>> listPendingTasks(String tableName) {
        ...
        jdbcTemplate.query("SELECT t.id, t.queue_name, substring(t.task for 2000) as payload, t.process_time, " +
                "t.attempt, t.create_time, t.log_timestamp as correlation_id, t.actor " +
                ...
                "on task_ids.id=t.id WHERE task_ids.rn<=10", rs -> {
     ...
     * @param tableName таблица с задачами
     */
    public void resetPendingTasks(String tableName) {
        jdbcTemplate.update(String.format("UPDATE %s SET process_time=now(), attempt = 0 " +
